...
export default function Navbar() {
    ...
    <Link
      ...
      className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium transition-colors
        ${location.pathname === to
          ? 'bg-blue-600 !text-white nav-blue-btn shadow-sm font-semibold'
          : 'text-slate-700 dark:text-slate-300 hover:text-blue-600 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800/80'}`}
    >
      {icon}
      {label}
    </Link>
  ...
  const themeToggleButton = (
    <button
      type="button"
      onClick={cycleTheme}
      className="p-2 mx-1 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-900/90 text-slate-700 dark:text-slate-200 hover:bg-slate-200 dark:hover:bg-slate-800 hover:border-slate-300 dark:hover:border-slate-600 transition shadow-sm cursor-pointer shrink-0"
    ...
    >
      {theme === 'light' ? (
        ...
        <Laptop className="h-4.5 w-4.5 text-slate-500 dark:text-slate-400" />
      )}
    </button>
  );
            ...
            <nav className="hidden md:flex items-center gap-1">
              {isAuthenticated ? (
                <>
                  ...
                  {navLink('/create-link', 'Create Link', <Link2 className="h-4 w-4 text-blue-600 dark:text-blue-400" />)}
                  ...
                  {balance !== null && (
                    <Link
                      to="/ledger"
                      className="flex items-center gap-1.5 px-3 py-1.5 mr-1 rounded-full bg-blue-50 dark:bg-blue-500/10 text-blue-700 dark:text-blue-400 font-bold text-xs hover:bg-blue-100 dark:hover:bg-blue-500/20 transition-colors border border-blue-200 dark:border-blue-500/20 shadow-sm"
                  ...
                  {/* Theme Switcher Button - Always Visible In Navbar */}
                  {themeToggleButton}
                  ...
                  <div className="relative" ref={dropdownRef}>
                    <button
                      ...
                      className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border text-sm font-semibold transition-all cursor-pointer ${
                        userMenuOpen
                          ? 'bg-blue-50 dark:bg-slate-800 border-blue-500 text-blue-700 dark:text-white shadow-sm'
                    ...
                    >
                      ...
                      <span className="max-w-[110px] truncate text-slate-800 dark:text-slate-200">{user?.name || user?.username || 'Account'}</span>
                      ...
                      <div className="absolute right-0 mt-2 w-64 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-2xl py-2 z-50 text-slate-800 dark:text-slate-200 backdrop-blur-xl animate-in fade-in slide-in-from-top-2 duration-150">
                        ...
                        <div className="pt-2 mt-1 border-t border-slate-100 dark:border-slate-800 px-1 space-y-1">
                          <button
                            type="button"
                            onClick={cycleTheme}
                            className="w-full flex items-center justify-between px-3 py-2 rounded-xl text-xs font-medium hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 transition-colors cursor-pointer"
                          >
                ...
                <>
                  {navLink('/shops', 'Shops', <Store className="h-4 w-4" />)}
                  {navLink('/developers', 'APIs', <Code className="h-4 w-4 text-emerald-600 dark:text-emerald-400" />)}
                  
                  <button
                    onClick={() => setShowTrackModal(true)}
                    className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium text-slate-700 dark:text-slate-300 hover:text-blue-600 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors cursor-pointer"
                  >
                    <MapPin className="h-4 w-4" />
                    Track Order
                  </button>
                  
                  {navLink('/help', 'Help', <HelpCircle className="h-4 w-4 text-blue-600 dark:text-blue-400" />)}
                  {navLink('/contact', 'Contact Us', <Phone className="h-4 w-4 text-emerald-600 dark:text-emerald-400" />)}
            ...
            {isAuthenticated ? (
              <>
                ...
                <div className="space-y-1 pt-2 border-t border-slate-200 dark:border-slate-800">
                  ...
                  {navLink('/developers', 'Developer APIs & SDK', <Code className="h-4 w-4 text-emerald-600 dark:text-emerald-400" />)}
                  <button
                  ...
                  {navLink('/help', 'Platform Help & Guides', <HelpCircle className="h-4 w-4 text-blue-600 dark:text-blue-400" />)}
                  {(user?.role === 'ADMIN' || user?.role === 'SUPPORT_AGENT') &&
              ...
              <div className="space-y-1">
                {navLink('/shops', 'Shops Marketplace', <Store className="h-4 w-4 text-blue-600 dark:text-blue-400" />)}
                {navLink('/developers', 'Developer APIs & SDK', <Code className="h-4 w-4 text-emerald-600 dark:text-emerald-400" />)}
                <button
                  onClick={() => { setShowTrackModal(true); setMenuOpen(false); }}
                  className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white transition-colors cursor-pointer"
                ...
                {navLink('/help', 'Platform Guide & Help', <HelpCircle className="h-4 w-4 text-blue-600 dark:text-blue-400" />)}
                {navLink('/contact', 'Contact Support', <Phone className="h-4 w-4 text-emerald-600 dark:text-emerald-400" />)}
