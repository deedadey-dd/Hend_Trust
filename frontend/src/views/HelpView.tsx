        ...
        <div className="max-w-4xl mx-auto text-center space-y-4">
          ...
          <div className="relative max-w-lg mx-auto pt-2">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3 bg-slate-950 border border-slate-700 rounded-2xl text-xs sm:text-sm text-white placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/20 shadow-xl"
            />
          </div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 pt-10 space-y-10">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          {[
            ...
            return (
              <div key={idx} className="bg-slate-900 border border-slate-800 p-4 rounded-2xl text-center space-y-1.5 shadow-lg">
        ...
        <div className="space-y-3">
          {filteredFaqs.length === 0 ? (
            <div className="text-center py-12 bg-slate-900 rounded-2xl border border-slate-800 p-6 text-slate-400 space-y-2">
              <ShieldAlert className="h-8 w-8 text-amber-400 mx-auto" />
              <p className="font-bold text-white text-sm">No topics match your search query.</p>
            ...
            filteredFaqs.map(faq => {
              ...
              return (
                <div key={faq.id} className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden transition shadow-md">
                  <button
                    onClick={() => setOpenFaqId(isOpen ? null : faq.id)}
                    className="w-full p-5 text-left flex items-center justify-between gap-4 hover:bg-slate-850 transition"
                  >
                    <div className="flex items-center gap-3">
                      <span className="bg-blue-500/10 border border-blue-500/20 text-blue-400 font-mono text-[10px] font-bold px-2.5 py-1 rounded-full uppercase">
                        {faq.category}
                      </span>
                      <span className="font-bold text-slate-100 text-xs sm:text-sm">{faq.question}</span>
                    </div>
                    {isOpen ? <ChevronUp className="h-4 w-4 text-blue-400 shrink-0" /> : <ChevronDown className="h-4 w-4 text-slate-500 shrink-0" />}
                  </button>
                  {isOpen && (
        ...
        <div className="bg-gradient-to-r from-blue-900/40 via-indigo-900/40 to-slate-900 border border-blue-500/30 p-6 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-4 shadow-xl">
          <div>
            <h3 className="text-base font-extrabold text-white flex items-center gap-2">
              <PhoneCall className="h-5 w-5 text-emerald-400" />
              Still Need Assistance?
            </h3>
            <p className="text-xs text-slate-400 mt-1">Our Ghana support team is ready to assist you 24/7 with any transaction queries.</p>
          </div>
          <Link
            to="/contact"
